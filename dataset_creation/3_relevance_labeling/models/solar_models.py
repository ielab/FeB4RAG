...
class SOLARModel(BaseModel):
    ...
    def generate_description(self, propmt, query, combined_snippet):
        ...
        if len(tokenized_doc) > 1024:
            tokenized_doc = tokenized_doc[:1024]
            truncated_doc = self.tokenizer.convert_tokens_to_string(tokenized_doc)
        else:
    ...
    def generate_answer(self, prompt, query, search_results, max_per_passage):

        #first need to make sure every passage is less than max_per_passage

        search_results_combined = ""
        for rank, result in enumerate(search_results):
            tokenized_doc = self.tokenizer.tokenize(result)
            if len(tokenized_doc) > max_per_passage:
                tokenized_doc = tokenized_doc[:max_per_passage]
                truncated_doc = self.tokenizer.convert_tokens_to_string(tokenized_doc)
            else:
                truncated_doc = result
            search_results_combined += "[" + str(rank + 1) + "]:\n" + truncated_doc + "\n"


        prompt_reformed = prompt.replace("{query}", query).replace("{search_results}", search_results_combined)
        #print(prompt_reformed)
        # print(prompt_reformed)
        input_ids = self.tokenizer(prompt_reformed, return_tensors="pt").input_ids.to(self.device)
        ...
        text_output = text.split("[/INST]Response:\n")[-1]
        # remove special token
